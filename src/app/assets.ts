...
export default function buildAssets(outdir: string): void {
  ...
  browserify()
    ...
    if (!fs.existsSync(`${outdir}/waveforms`)) {
      fs.mkdirSync(`${outdir}/waveforms`);
    }

    try {
      fs.cpSync(`${__dirname}/../../waveforms`, `${outdir}/waveforms`, { recursive: true });
      console.log(colors.cyan("[asset] waveforms directory built"))
    } catch (err) {
      console.error(err);
    }
}
